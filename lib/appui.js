            ...
            case 'audio-call':
                ...
                if(window.callData) {
                    ...
                    if (!$('#call .forShowChatPos').children().length) {
                        const cantainer = $('<div id="'+params.contact.pkeyhash+'"></div>');
                        cantainer.append('<h3>'+params.contact.name+'</h3>');
                        ...
                        cantainer.append('<div style="text-align: center;">\
                                            <canvas class="audio-viz" style="background-color: #666;height:220px;"></canvas>\
                                          </div>');
                ...
                if(window.callData) {
                    if (smallMediaContainer.css('display') !== 'block') {
                        $.each($('.forShowChatPos > div'), (eix, el) => {
                            ...
                            excerpt.append($('<span></span>').text($(el).find('h3').text()));
                            excerpt.append($('<div></div>').append($(el).find('.remote-audio')).css({ position: 'absolute', opacity: 1, height: 1, bottom: 30 }));
                            excerpt.append($('<div></div>').css({ float: 'none',  }).append($(el).find('.audio-viz')));
                            excerpt.append($('<div></div>').html('').css({ float: 'none', clear: 'both', width: '100%', height: 2, background: '#c4c4c4' }));
                        ...
                        smallMediaContainer.find('canvas').css({ width: '100%', height: 160 });
