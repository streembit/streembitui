...
function MainVM() {
    ...
    function validate_page(param) {
        ...
        if (page == "connectpublic") {
            ...
            if (!appsrvc.username || !appsrvc.account_connected) {
                streembit.notify.error("First initialize the account by connecting to the City of Osmio P2P network");
                return "initui";
            }

            var curr_account = accounts.get_account(appsrvc.username);
            if (!curr_account) {
                streembit.notify.error("The account is not initialized. First initialize the account by connecting to the City of Osmio P2P network");
                page = "initui";
            }
        }
    ...
    var viewModel = {
        ...
        offline_contact_request: function () {            
            if (!appsrvc.account_connected) {
                return streembit.notify.error("First connect to the City of Osmio P2P network");
            }
