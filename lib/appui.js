                ...
                if(window.callData) {
                    if (smallMediaContainer.css('display') !== 'block') {
                        $.each($('.forShowChatPos > div'), (eix, el) => {
                            ...
                            excerpt.append($('<div></div>').append($(el).find('.remote-audio')).css({ position: 'absolute', opacity: 0, height: 1 }));
                            excerpt.append($('<div></div>').css({ float: 'right',  }).append($(el).find('.audio-viz')));
                        ...
                        smallMediaContainer.find('canvas').css({ width: '100%', height: 32 });
