    ...
    var videoPanel = {
        update: function(element, valueAccessor, allBindings) {
            ...
            if (page != "video-call" && window.callData && window.callData.calltype == "videocall") {
                ...
                if (!valueUnwrapped) {
                    value(true);
                    var elementContainer = element.querySelector('.sm-video-container')
                    var smVideo = document.querySelector(".remotevid");

                    // we only need to save the remote video, to not loos the video stream
                    elementContainer.innerHTML = "";
                    elementContainer.appendChild(smVideo);
                    elementContainer.querySelector('video').play();
                }
            }
            ...
            if(page == "video-call" && window.callData) {  //if you are in the call view page
                setTimeout(function () {
                    if(valueUnwrapped) {
                        var smVideo = element.querySelector(".remotevid");
                        var callViewVideo = document.querySelector('.remotevid_parent');

                        callViewVideo.innerHTML = "";
                        callViewVideo.appendChild(smVideo);
    ...
    var audioPanel = {
        update: function(element, valueAccessor, allBindings) {
            ...
            if (page != "audio-call" && window.callData && window.callData.calltype == "audiocall") {
                ...
                if (!valueUnwrapped) {
                    value(true);
                    var AudioPanel = element.querySelectorAll('.remote-audio-wrapper');
