...
var Config = {
    ...
    init: function () {
        if (!configdef) {
            ...
            throw new Error(errhandler.getmsg(errcodes.UI_CONFIG_FILE_IS_MISSING));
        }

        if (!configdef.appconfig) {
            // throw new Error("appconfig section is missing from config.json configration");
            throw new Error(errhandler.getmsg(errcodes.UI_APPCONFIG_SECTION_IS_MISSING));
        }

        streembit.globals.nwmode = addconfig.nwmode == false ? false : true;

        if (addconfig.protocol == "https") {
            streembit.globals.protocol = addconfig.protocol;
        }
    }
}
