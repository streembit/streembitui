...
function IoTHub(host, port, id, publickey, devices) {
    this.protocol = "wss"; // default is wss
    this.wssocket = 0;
    this.host = host;
    this.port = port;
...
function IoTHubsHandler(address, port) {
    ...
    this.authenticate = function (id, devices, callback) {
        try {
            ...
            hub.sendauth(message, function (err, response) {
                ...
                /*
                let sha1st = createHash("sha512").update(this.authtoken).digest("hex");
                this.symmcryptkey = createHash("sha512").update(sha1st).digest("hex");
                */
    ...
    this.inithub = function (device, callback) {
        ...
        this.create_connection(id, publickey, port, host, devices, (err) => {
            if (err) {
