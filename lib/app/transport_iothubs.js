...
function IoTHub(host, port, protocol, id, publickey, devices) {
    ...
    this.send = function (message, callback) {
        ...
        message.txn = txn;
        message.pkhash = appsrvc.pubkeyhash;
...
function IoTHubsHandler(address, port) {
    ...
    this.gethostinfo = async function (id, publickey, callback) {
        //TODO get the host info from hte DHT or from the centralised Streembit discovery services

        try {
            const device = await database.get(database.IOTDEVICESDB, id);

            if (!device) {
                throw new Error('Device not found by ID');
            }

            const key = `${device.publickey}/${appsrvc.pubkeyhash}`;
            const result = await transport.get(key);
            if (!result) {
                throw new Error('Device not found in DHT');
            }
            ...
            if (!payload || !payload.data || !payload.data.type || payload.data.type != peermsg.MSGTYPE.CAMSG) {
                return callback("get_contact_offer error: invalid device payload");
            }
            ...
            if (isspkey !== publickey) {
                return callback("get host info error: public key mismatch with device: " +publickey+ ", new request is required");
            }
            ...
            if (!connection) {
                return callback("gethostinfo error: no connection details field is published for " + device.publickey);
            }

            if (connection.account != account) {
                return callback("gethostinfo error: account mismatch was published for " + device.publickey);
            }

            var host = connection[peermsg.MSGFIELD.HOST];
            if (!host) {
                return callback("gethostinfo error: no address field is published for " + device.publickey);
            }

            var port = connection[peermsg.MSGFIELD.PORT];
            if (!port) {
                return callback("gethostinfo error: no port field is published for " + device.publickey);
            }

            var protocol = connection[peermsg.MSGFIELD.PROTOCOL];
            if (!protocol) {
                return callback("gethostinfo error: no protocol field is published for " + device.publickey);
            }
...
console.log('PROTO', protocol);
    ...
    this.init = function () {
        try {
            ...
            streembit.notify.error(errhandler.getmsg(errcodes.UI_IOTHUBSHANDLER_INIT_ERR, e, true));
        }
    };
}
