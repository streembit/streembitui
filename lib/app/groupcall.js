...
(function () {
    ...
    call.webrtcadd = function (partee, type = null, iscaller = true) {
        ...
        const webrtccaller = Object.assign({}, webrtccall);
    ...
    call.initcall = function(payload) {
        try {
            ...
            call.sid = data[peermsg.MSGFIELD.GSID];
            const taskid = uuid.v1();
            connections.create(data.attendee, true, taskid, err => {
                appevents.dispatch("on-task-event", "close", "send", taskid);

                if (err) {
                    return streembit.notify.error('Error creating connection with ' +data.attendee.name);
                }
            ...
            streembit.notify.error('Initcall error, %j', err);
        }
        // try {
        //
        //     const data = JSON.parse(payload);
    ...
    call.readycall = function (payload) {
        ...
        try {
            //const taskid = uuid.v1();
            //connections.create(caller[0], false, taskid, err => {
                //appevents.dispatch("on-task-event", "close", "send", taskid);

                //if (err) {
                //    return streembit.notify.error('Error creating connection with ' +caller[0].attendee.name);
                //}

                call.webrtcadd(caller[0], null, false);
            //})
        } catch (err) {
            streembit.notify.error('Ready call error, %j', err);
        }
    };
