...
(function () {
    ...
    call.calltimeproc = function (contact) {
        ...
        call.call_timer_obj[contact.pkeyhash] = setInterval(function () {
            ...
            if(document.querySelector("[data-calltime= '" +  contact.pkeyhash + "'] .call-time" )) {
                document.querySelector("[data-calltime= '" +  contact.pkeyhash + "'] .call-time" ).innerHTML = txt;
            }
        }, 1000);
    };
    ...
    call.hangupCall = function (partee) {
        try {
            ...
            if (call.call_timer_obj) {
                for(var index in call.call_timer_obj) {
                    clearTimeout(call.call_timer_obj[index]);
                }
            }
        }
        catch (e) {
    ...
    call.hangupAll = function () {
        for(var i = call.active.length - 1; i >= 0; i--) {
            call.hangupCall(call.active[i]);
        }
        // for(var index in call.active) {
        //     call.hangupCall(call.active[index]);
        // }
    };
                        ...
                        function (result) {
                            ...
                            if (result) {
                                ...
                                webrtccall.initcall(null, null, options, err => {
                                    ...
                                    for(var index in call.active) {
                                        setTimeout(() => {
                                            peercomm.group_add(call.active[index], defs.CALLTYPE_AUDIO, sid, item);
                                        }, 500);
                                    }
                                    call.add(item);
                                ...
                                //TODO: group call - add the new partee to small panel if you are not in the call view
                                // var newParticipant = `<div class="audio-container" data-bind="attr: { id: pkeyhash }">
