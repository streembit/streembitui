            ...
            if (obj.offset >= obj.size ) {
                if (streembit.globals.nwmode1) { // TODO the corect is nwmode, nwmode1 is for test
                    ...
                    //crypto.subtle.digest("SHA-1", buffer).then((result) => {
                        ...
                        } else if (obj.actiontype === "chatimg") {
                            appevents.dispatch("oncontactevent", "on-media-message", new Blob(obj.blobitems, { type: obj.type }), 'image', contact);
                        }
            ...
            try {
                ...
                file.read(function (err, result) {
                    ...
                    if (!result) {
                        ...
                        return onerror(errhandler.getmsg(errcodes.UI_FAILED_RESEIVE_DATA_FROM_FILEREAD));
                    }
