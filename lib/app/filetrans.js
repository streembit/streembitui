...
(function () {
    ...
    function fileItem(fileobj, filename, filesize, filehash, filetype, filepath, chunksize) {
        ...
        function fwrite(pos, offset, length, chunk, callback) {
        ...
        obj.write = function (pos, offset, length, chunk, callback) {
            try {
                ...
                //console.log("file write pos: " + pos + " offset: " + offset);

                fwrite(pos, offset, length, chunk, function (err) {
                    if (err) {
                        return callback(err);
                    }

                    if (!streembit.globals.nwmode1) { // TODO change to nwmode if the nw file proces is implemented
    ...
    function sendproc(hash, onread, oncomplete, onerror) {
        ...
        var path = file.path;
        if (!path) {
            return onerror("File send error: invalid file path");
        }
