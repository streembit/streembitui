...
(function () {
    ...
    function updateDB(contact) {
        database.ContactsDB.update_contact(user.name, contact).then(
            function () {
            },
            function (err) {
    ...
    handler.update_contact_database = function (contact, callback) {
        ...
        database.ContactsDB.update_contact(user.name, updobj).then(
            function () {
                for (var i = 0; i < contacts.length; i++) {
    ...
    handler.remove = function (name, callback) {
        database.ContactsDB.delete_contact(user.name, name, function (err) {
            if (err) {
    ...
    handler.init = function (callback) {
        try {
            ...
            database.ContactsDB.get_contacts(user.name, function (err, result) {
                if (err) {
