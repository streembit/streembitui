...
(function () {
        ...
        apputils.get_pending_contacts(function (err, pcontacts) {
            ...
            var pctimer = setInterval(
                function () {
                    ...
                    // check if the contact is already exists
                    if (contactlist.exists(account)) {
                        // delete the contact from the pending contacts database
                        ...
                        handler.search(account, function (contact) {
                            if (contact) {
                                handler.send_addcontact_request(contact);
                            }
                            else {
                                apputils.delete_pending_contact(account, function () {
                                });
    ...
    function get_account_msgs() {
        var key = appsrvc.username + "/message";
        peercomm.get_account_messages(key, null, null, function (err, result) {
            if (err) {
                return appevents.emit(appevents.ERROREVENT, "findmessages", err);
            }
    ...
    handler.handle_addcontact_accepted = function (account) {
        var contact = apputils.get_pending_contact(account);
        if (contact) {
            contactlist.update_contact(contact, function () {
    ...
    handler.send_addcontact_request = function (contact) {
        ...
        apputils.add_pending_contact(contact, function (err) {
            ...
            setTimeout(
                function () {
                    var pendingc = apputils.get_pending_contact(account);
                    if (pendingc) {
    ...
    handler.load = function () {
        return new Promise(function (resolve, reject) {
            ...
            appevents.addListener("offline-addcontact-accepted", handler.offline_addcontact_accepted);
            appevents.addListener("offline-addcontact-declined", handler.offline_addcontact_declined);
            appevents.addListener("on-send-addcontact-request", handler.send_addcontact_request);
