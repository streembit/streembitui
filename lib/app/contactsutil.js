...
(function () {
    ...
    handler.add_contact = function (contact, cbfunc) {
        ...
        function (err) {
            if (err && !isoffer_complete) {
                return cbfunc("Error in finding contact details and creating contact offer: " + (err && err.message) ? err.message : err);
            }
    ...
    handler.search = function (account, callback) {
        try {
            ...
            peercomm.get_contact_offer(account, function (err, contact) {
                if (err) {
