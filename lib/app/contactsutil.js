...
(function () {
    ...
    // TODO execute this when the contact list is initialized
    handler.send_contact_offer = function (contact, cbfunc) {
        try {
            peercomm.send_contact_offer(
                ...
                (err) => {
                    if (err) {
                        return streembit.notify.error("Error in sending contact offer: %j", err, true);
                    }

                    // TODO notify the UI and contact list that the contact offer was sent

                    logger.debug("Contact offer was sent to " + contact.name)
                });
        }
        catch (err) {
            streembit.notify.error("Error in sending contact offer: %j", err, true);
        }
    }    
