        ...
        var handler = {
                ...
                .then(
                    ...
                    function (isaccepted) {
                        ...
                        if (isaccepted == true) {
                            ...
                            streembit.notify.info(errhandler.getmsg(errcodes["UI_ACCEPT-FILE"]));
                ...
                try {
                    ...
                    handler.offer(params, function () {
                        try {
                            ...
                            webrtcfile.sendfile(params, (isInProgress => () => {
                                isInProgress = false;
                            })());
                            //
                        }
            ...
            readfile: function (recipient, actiontype = 'file') {
                try {
                    ...
                    if (file.size > defs.MAX_FILE_SIZE) { // 25Mb
                        return alert('File is too big, please do not exceed 10Mb of size');
                    }
                    ...
                    var blobSlice = File.prototype.slice || File.prototype.mozSlice || File.prototype.webkitSlice,
                        chunkSize = defs.FILE_TRANS_CHUNK_SIZE, // Read by chunks of 1024Kb/1Mb
                        chunks = Math.ceil(file.size / chunkSize),
                        currentChunk = 0,
                        array = [];

                    const fileOnload = function (e) {
                        try {
                            ...
                            callback: function () {
                                if(recipients.constructor === Array) {
                                    for (var i in recipients) {
                                        handler.readfile(recipients[i], actiontype);
                                    }
                                } else {
                                    handler.readfile(recipients, actiontype);
                                }
                            }
                        }
                    }
