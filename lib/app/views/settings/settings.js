...
(function () {
    ...
    define(['i18next', 'settings', 'logger', 'definitions', 'errhandler', 'errcodes', 'apputils', './settings.html!text'],
        function (i18next, settings, logger, defs, errhandler, errcodes, apputils, template) {
            ...
            function SettingsVm() {
                    ...
                    init: function (callback) {
                        try {
                            ...
                            settings.getdata(function (err, data) {
                                try {
                                    if (err) {
                                        return streembit.notify.error(errhandler.getmsg(errcodes.UI_SETTINGS_INIT, err));
                                    }
                                    ...
                                    self.peers_remote_url(data.kadence_peerlist || data.wspeerlisturl);

                                    if (callback) {
                                        callback();
                                    }

                                    const ping = function ping(item) {
                                        try {
                                ...
                                //
                                    ...
                                    streembit.notify.error(errhandler.getmsg(errcodes.UI_SETTINGS_INIT, err));
                ...
                save: function () {
                    try {
                        ...
                        var wspeers = [];
                        for (var i = 0; i < viewModel.bootseeds().length; i++) {
                            wspeers.push({ host: viewModel.bootseeds()[i].host, port: viewModel.bootseeds()[i].port});
                        }

                        data.bootseeds = wspeers;
                        ...
                        data.isaccountexists = settings.isaccountexists;

                        data.wspeerlisturl = viewModel.peers_remote_url();

                        settings.update(data, function (err) {
                            if (err) {
                                streembit.notify.error(errhandler.getmsg(errcodes.UI_UPDATE_SETTINGS_DB, +err.message));
                            }
                            else {
                        ...
                        return streembit.notify.error(errhandler.getmsg(errcodes.UI_EXCEPTION_OCCURED_UPD_SETT_DB, +e.message));
                    }
                },
                ...
                add_bootseed: function () {
                    ...
                    if (!seed_host || !seed_port || isNaN(parseInt(seed_port))) {
                        return streembit.notify.error(errhandler.getmsg(errcodes.UI_INVALID_SEED_DATA));
                    }
