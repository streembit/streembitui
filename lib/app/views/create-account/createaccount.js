...
(function () {
    ...
    define(
        ['i18next', 'appevents', 'apputils', 'user', 'peercomm', 'appsrvc', 'definitions', 'uihandler', 'errhandler', 'errcodes', 'accounts', 'settings', './createaccount.html!text'],
        function (i18next, appevents, apputils, user, peercomm, appsrvc, defs, uihandler, errhandler, errcodes, accounts, settings, template) {
        ...
        function CreateAccountVm(params) {
            ...
            this.ismnemonic_visible = (params && params.mode && params.mode === 'mnemonic') ? ko.observable(true) : ko.observable(false);
            ...
            this.cancel_normal = function () {
                if (accounts.list.length) {
                    return appevents.navigate("initaccount");
                }

                return streembit.notify.error(errhandler.getmsg(errcodes.UI_CREATE_ACCOUNT));
            };
                ...
                try {
                    ...
                    user.create_key(password, this.valid_mnemonic(), function (err) {
                        if (err) {
                            ...
                            return streembit.notify.error(errhandler.getmsg(errcodes.UI_ERROR_CREATING_ACCOUNT, err));
                        }

                        user.save_account(account, function (err) {
                            if (err) {
                                uihandler.unblockwin();
                                return streembit.notify.error(errhandler.getmsg(errcodes.UI_ERROR_SAVING_ACCOUNT, err));
                            }

                            settings.update_accountexists(true, function (err) {
                                if (err) {
                                    uihandler.unblockwin();
                                    return streembit.notify.error(errhandler.getmsg(errcodes.UI_UPDATE_SETTINGS, err));
                                }
                    ...
                    streembit.notify.error(errhandler.getmsg(errcodes.UI_ERROR_CREATING_ACCOUNT_USER, err));
                }
            };
