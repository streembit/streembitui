...
(function () {
    ...
    define(
        ['i18next', 'appsrvc', 'user', 'appevents', 'accounts', './changepwd.html!text' ],
        function (i18next, appsrvc, user, appevents, accounts, template) {

            function ChangePwdVm(params) {

                if (!appsrvc.username) {
                    appevents.navigate("dashboard");
                    return streembit.notify.error("To change the password first you must initialize your account");
                }
                ...
                this.submit = function () {
                    try {
                        ...
                        var self = this;
                        user.change_password(this.accountname, password, function (err) {
                            if (err) {
