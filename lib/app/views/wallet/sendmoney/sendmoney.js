...
(function () {
    define(
        ['i18next', 'definitions', 'appevents', 'uihandler', './sendmoney.html!text'],
        function (i18next, definitions, appevents, uihandler, template) {
            function SendMoneyModel(params) {
                ...
                var viewModel = {
                    ...
                    sendComplete: function (err, result) {
                        ...
                        if (err || !result.dispatched) {
                            streembit.notify.error(err ? err : 'Sending transaction to blockchain failed');
                        } else {
                            streembit.notify.success('The transaction was sent to the blockchain');
                            viewModel.clear();                            
                            $('a[href="#mywallet_overview"]').trigger('click');
                        }
                    }
                };
