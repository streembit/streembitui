    ...
    define(
        ...
        function (i18next, appevents, settings, template) {
            ...
            function WalletModel(params) {
                ...
                this.walletBalanceLabel = i18next.t('view-title-balance');
                this.walletBalanceAmount = ko.observable(0);
                this.walletBalanceInfo = ko.pureComputed(() => {
                    return this.walletBalanceLabel+ ': ' +this.walletBalanceAmount();
                }, this);
                ...
                this.onBcEvent = function (payload, event) {
                    ...
                    if (payload.error !== false) {
                        // notify user with
                        // payload.error: {no: 123456, msg: "error msg"}
                        appevents.dispatch("on-activity", {
                            type: 'error',
                            msg: `Operation: ${payload.event}. Status: ERROR. ${payload.error.no}. ${payload.error.msg}`
                        });
                        return;
                    }
                    ...
                    switch(payload.event) {
                        case 'sendresult':
                                msg = `Send Money. Status: SUCCESS, TXID: ${payload.txid}`;
                            break;
                        default:
                            return;
                            break;
                ...
                this.restore = function(){
                    ...
                    box.init(function () {
                        ...
                        $(".modal-dialog").addClass('forAllModalInWalletContent');
                    });
                }
                ...
                this.signMessageItem = function(){
                    ...
                    var box = bootbox.alert({
                        message:  content ,
                        title: "Signatures - Sign / Verify a Message",
                        size: 'large',
                        closeButton: true,
                        backdrop: true,
                        callback: function(){}
                ...
                this.veifyMessageItem = function () {
                    var content = $('#signmessage-item-template').html();
                    var box = bootbox.dialog({
                        message: content,
