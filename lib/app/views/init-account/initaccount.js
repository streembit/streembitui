...
(function () {
    ...
    define(
        ...
        function ConnectPublicVm(params) {
            ...
            this.initialization_progress = ko.observable(false);
            ...
            //     });
            // };
            this.restore_dialog = function (data, event) 
            {
              ...
              if (event.target.value === 'backup') {
                $('#restore-window-modal').modal('hide');
                $('body').removeClass('modal-open');
                $('.modal-backdrop').remove();
                self.restore_account();
            } else if (event.target.value === 'mnemonic') {
                $('#restore-window-modal').modal('hide');
                $('body').removeClass('modal-open');
                $('.modal-backdrop').remove();
                appevents.navigate("createaccount", { mode: 'mnemonic' });
            ...
            this.initialize_account = function () {
                this.initialization_progress(true);
                $('#load-container').show().find('#load-info').text('Initializing Account ...');
                
                var timer = 0, processed = false;
                try {
                    var accobj = this.validateAccountText(),
                        password = this.onPasswordChange();
                    if (!accobj || !password) {
                        $('#load-container').hide();
                        return this.initialization_progress(false);
                    }
                    
                    // initialize account
                    setTimeout(() => user.initialize(accobj, password, err => {
                        $('#load-container').hide();
                        
                        if (err) {
                            this.initialization_progress(false);
