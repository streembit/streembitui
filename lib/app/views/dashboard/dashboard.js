...
(function () {
    define(['i18next', 'appevents', 'appsrvc', 'uihandler', 'contactlist', 'user', './dashboard.html!text'],
        function (i18next, appevents, appsrvc, uihandler, contactlist, user, template) {
        function DashboardViewModel(params) {
            var viewModel = {
                device_count: ko.observable(0),
                contactsCount: ko.observable(),
                coin_count: ko.observable(0),
                contactName: ko.observable(''),

                init: function () {
                    console.log("dashboard init");
                    $('body').addClass('nav-function-fixed')
                    viewModel.contactsCount(contactlist.contacts.length + ' ' + 'contacts');

                    viewModel.contactName(appsrvc.username);
