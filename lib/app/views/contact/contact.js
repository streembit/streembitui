...
(function () {
    define(
        [
            'appevents', 'peercomm', 'logger', 'peermsg', 'appsrvc', 'definitions', "connections", "screenshare", "webrtccall", "groupcall", "uuid", "webrtcscreen",
            'uihandler', 'filesender', 'utilities', 'contactlist', 'database', 'errhandler', 'errcodes', './contact.html!text'
        ],
        function (
            appevents, peercomm, logger, peermsg, appsrvc, defs, connections, screenshare, webrtccall, groupcall, uuid, webrtcscreen,
            uihandler, filesender, utilities, contactlist, database, errhandler, errcodes, template) {
        ...
        function ContactVM(params) {
            ...
            var viewModel = {
                ...
                hangup: function (type) {
                    $('.gradient-titanium #hangupCallFromCallProgress').on('click', function () {
                        viewModel.errorCallAndHangup(true);
                        appevents.dispatch("on-appui-event", "hide-contacts-bar", viewModel.contact);
                        return peercomm.hangup_call(viewModel.contact, null, {msg : 'hangup'});
                    })
                },
                ...
                ping: function () {
                    ...
                    peercomm.ping(this.contact, true, 10000).then(
                        ...
                        (err) => {
                            ...
                            streembit.notify.error(errhandler.getmsg(errcodes.UI_ERROR_IN_PINGING_CONTACT, err));
                    ...
                    taskTimer = setTimeout(function () {
                        ...
                        if (!pinged && !reported) {
                            ...
                            streembit.notify.error(errhandler.getmsg(errcodes.UI_ERROR_IN_PINGING_CONTACT, null, true));
                    ...
                    try {
                            ...
                            function () {
                                ...
                                webrtcscreen.share(
                                    ...
                                    function (err) {
                                        ...
                                        streembit.notify.error(errhandler.getmsg(errcodes.UI_SHARING_SCREEN, err));                                       
                                        appevents.dispatch("on-task-event", "close", "send", taskid);
                                        viewModel.make_connection_inprogress = false;
                                    }
                                );
                        ...
                        taskTimer = setTimeout(function () {
                            if (!created && !reported) {
                                ...
                                streembit.notify.error(errhandler.getmsg(errcodes.UI_NO_REPLY_FROM_CONTACT, null, true));
                                appevents.dispatch("on-task-event", "close", "send", taskid);
                                viewModel.make_connection_inprogress = false;
                            }
                        },
            ...
            //TODO: check all the participants names in case of group call
            if(window.callData && window.callData.contact.name === params.name) {
                ...
                else if(window.callData.calltype === "videocall") {
                    appevents.navigate('video-call', window.callData);
                }
            }
