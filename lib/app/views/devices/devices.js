    ...
    define(['appevents', 'database', 'logger', 'definitions', 'async', './view.html!text'], function (appevents, database, logger, defs, async, template) {
        ...
        class GatewayDevice extends Device {
            constructor(data) {
                ...
                this.notpermitted_devices = ko.observableArray([]);
            ...
            cancel_edit_notpremitted_devices() {
                this.isedit_mode(false);
                this.template_name('empty-template');
            }

            submit_notpremitted_devices() {
                this.isedit_mode(false);
                this.template_name('empty-template');
            }
                                ...
                                rcvdevicelist.forEach(
                                    (device) => {
                                        if (device.deviceid == this.id) {
                                            ...
                                            existing_devices.forEach(
                                                (currdevice) => {
                                                    if (device.deviceid == currdevice.id) {
                                                        isconfigured = true;
                                                    }
                                                }
                                            );
                                            ...
                                            if (!isconfigured) {
                                                ...
                                                if (device.protocol == "zigbee") {
                                                    device.descriptors.forEach(
                                                        (descriptor) => {
                                                            let clusters = descriptor.clusters;
                                                ...
                                                this.notpermitted_devices.push(device);
                                            }
                                        }
                                    }
                                );

                                if (this.notpermitted_devices().length > 0) {
                                    // show the list of not permitted devices for the user
                                    this.template_name("notpermitted-" + protocol + "-devices-template");
