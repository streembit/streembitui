...
(function () {
    define(['appevents', 'database', 'logger', 'definitions', 'async', './view.html!text'], function (appevents, database, logger, defs, async, template) {
        ...
        class ElectricityMeasurement extends Feature {
            ...
            get_report_config() {
                return { feature: this.type, interval: defs.MIN_REPORTING_INTERVAL };
            }
        ...
        class Device {
            ...
            set_zigbee_netinfo(response) {
                this.networkinfo.manufacturername(response.details.manufacturername);
                this.networkinfo.modelidentifier(response.details.modelidentifier);
                this.networkinfo.hwversion(response.details.hwversion);
                this.networkinfo.address64(response.details.address64);
                this.networkinfo.address16(response.details.address16);
            }
            ...
            set_gateway_netinfo(response) {
                ...
                this.networkinfo.hwversion(response.details.hwversion);
                this.networkinfo.address64(response.details.address64);
                this.networkinfo.address16(!response.details.address16 ? '0000' : response.details.address16);
                this.networkinfo.protocol(response.details.protocol);
                this.networkinfo.security(response.details.security);
                this.networkinfo.secure_hardware(response.details.secure_hardware);
            }
            ...
            configure_report(reports) {
                try {
                    var message = {
                        id: this.deviceid,
                        cmd: defs.IOTCMD_CONFIGURE_REPORT,
                        reports: reports
                    };
                    this.send(message, (err, response) => {
                        try {
                            ...
                            if (!response || !response.report_created) {
                                // report the error
                                return super.report_error("Configuring IoT report failed");
                            }

                            console.log("IoT report configured for device " + this.id + " configured")
            ...
            init() {
                ...
                setTimeout(
                    () => {
                        var reports = [];
                        for (var i = 0; i < features.length; i++) {
                            var report = features[i].get_report_config();
                            if (report) {
                                reports.push(report);
                            }
                        };
