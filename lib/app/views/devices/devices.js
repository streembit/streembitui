    ...
    define(['appevents', 'database', 'logger', 'definitions', 'async', './view.html!text'], function (appevents, database, logger, defs, async, template) {
        ...
        class Feature {
            constructor(data, device) {
                ...
                this.datareceived = ko.observable(false);
            }
        ...
        class ZigbeeNetInfo {
            constructor(data) {
                ...
                this.address16 = ko.observable(0);
                this.iddle_enable = ko.observable(0);
                this.relationship = ko.observable(0);
                this.permitjoin = ko.observable(0);
                this.depth = ko.observable(0);
                this.lqi = ko.observable(0);
            }
        }
        ...
        class Device {
            constructor(data) {
                ...
                if (data.network == defs.IOT_NETWORK_ZIGBEE) {
                    var netdata = {
                        address64: this.id
                    };
            ...
            set_zigbee_netinfo(response) {
                ...
                this.networkinfo.iddle_enable(response.details.iddle_enable);
                this.networkinfo.relationship(response.details.relationship);
                this.networkinfo.permitjoin(response.details.permitjoin);
                this.networkinfo.depth(response.details.depth);
                this.networkinfo.lqi(response.details.lqi);
            }
            ...
            set_netinfo(response) {
                ...
                if (this.network == defs.IOT_NETWORK_ZIGBEE) {
                    this.set_zigbee_netinfo(response);
                }
            }

            report_error(err) {
                streembit.notify.error("IoT device error %j", err);
            }
