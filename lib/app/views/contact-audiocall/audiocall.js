...
(function () {
    define(
        [
            ...
            'webrtccall', 'webrtcdata', 'filesender', 'uuid', 'secure-random', 'connections', 'errhandler', 'errcodes', './audiocall.html!text'
        ],
        function (
            appevents, contactlist, peercomm, appsrvc, defs, utilities, groupcall,
            webrtccall, webrtcdata, filesender, uuid, secrand, connections, errhandler, errcodes, template) {
            ...
            function showChatControl() {
                ...
                var textAudDur = $('#call .forShowChatPos').find('div');
                var img = textAudDur.find('img').css({'display': 'none'});
                $('#smallScreenAudioCall1').addClass('changestylesmall');
                $('#smallScreenAudioCall1 span').append(textAudDur);
                var audioCh = $('#smallScreenAudioCall1 span div').find('audio');
                $('.changestylesmall').find('canvas').css({'display':'none'});
                if(!$('.changestylesmall > .forSmScreen').siblings('div.smSpeakSt').length) {
                    $('.changestylesmall > .forSmScreen').after('<div class="smSpeakSt" style="text-align:center"><i class="fa fa-volume-up" style="font-size:45px;color:#6893c7"></i></div>');
                }
                $('#smallScreenAudioCall1 span div:eq(1)').after(audioCh);
                $('#smallScreenAudioCall1').show();
                $('#smallScreenAudioCall1 span div:eq(1)').next()[0].play();
            ...
            function AudioCallVM(params) {
                var viewModel = {
                    ...
                    init: function () {
                        ...
                        $('.forShowChatPos').find('canvas').css({'display': 'block'});
                        ...
                        setTimeout(() => {
                            ...
                            $.each(activeAu, (eix, el) => {
                                console.log('eL::', '[data-invite="' + $(el).attr('id') + '"]');
                                console.log('L::', $('[data-invite="' + $(el).attr('id') + '"]').length);
