        ...
        function (
            ...
            function AudioCallVM(params) {
                var viewModel = {
                    ...
                    group: ko.observableArray(groupcall.participants),
                    secid: null,
                    ...
                    init: function () {
                        ...
                        const webrtccaller = groupcall.getWebrtcCall(this.contact.pkeyhash);
                        ...
                        webrtccaller.initcall(null, null, options, err => {
                            ...
                            // if (groupcall.recipient) {
                            //     $.each(groupcall.pending, (idx, p) => {
                            //         groupcall.updateDOM(p, 'add');
                            //
                            //         options.elnum = idx + 1;
                            //         options.contact = p;
                            //         groupcall.getWebrtcCall(p.pkeyhash).initcall(null, null, options, err => {
                            //             if (err) {
                            //                 return streembit.notify.error('Start WebRTC call error, %j', err);
                            //             }
                            //
                    ...
                    hangup_gcpart: function () {
                        const pkeyhash = $(this).parent().attr('id');
                ...
                viewModel.groupcallInvite = ko.computed(() => viewModel.contacts().filter(c => !groupcall.participants.filter(g => c.name === g.name).length));
