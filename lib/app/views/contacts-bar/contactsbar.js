        ...
        function ContactsListVm() {
            ...
            var viewModel = {
                ...
                dispose: function () {
                    ...
                    appevents.remove(appevents.CONTACTS, this.onContactsUpdate);
                },

                init: function (list) {
                    // add event listners
                    appevents.onContactsEvent(this.onContactsUpdate);
                ...
                onContactsUpdate: function (event, data, payload) {
                    switch (event) {
                        case "receiveadd":
                            this.onReceiveAddContact(data);
                        case "addcontact":
                            this.add_contact(data);
                            break;
                        case "updatecontact":
                            this.update_contact(data, payload);
                            break;
                        default:
                            break;
                    }
                ...
                remove: function (item) {
                    bootbox.confirm("Contact '" + item.name + "' will be removed from the contacts list.", function (result) {
                        if (result) {
                            contacts_handler.remove(item.name, function () {
                ...
                search: function () {
                    try {
                        ...
                        if (contacts_handler.exists(account)) {
                            return streembit.notify.info("This contact is already exists in the contact list");;
                        }

                        contacts_handler.search(account, function (contact) {
                            if (contact) {
                                self.contact_lookup("");
                ...
                onSendAddContactRequest: function () {
                    contacts_handler.send_addcontact_request(viewModel.addcontact_obj, function () {
                        viewModel.addcontact_name("");
                        viewModel.addcontact_obj = 0;
                        viewModel.show_addcontact_panel(false);
                    });
                ...
                acceptAddContact: function (obj) {
                    ...
                    contacts_handler.accept_contact(contact);
                },

                declineAddContact: function (obj) {
                    var contact = obj.contact;
                    contacts_handler.decline_contact(contact);
                    var account = contact.name;
                    viewModel.recent_messages.remove(function (item) {
                        return item.contact && item.contact.name == account;
                    })
