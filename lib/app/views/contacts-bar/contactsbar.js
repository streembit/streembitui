...
(function () {
    ...
    define(
        ['appsrvc', 'contactlist', 'connections', 'uuid', 'peercomm', 'appevents', 'apputils', 'secure-random', 'settings', 'definitions', 'peermsg', 'contactsutil', 'uihandler', 'utilities', 'database', './contactsbar.html!text'],
        function (appsrvc, contactlist, connections, uuid, peercomm, appevents, apputils, secrand, settings, defs, peermsg, contactsutil, uihandler, utilities, database, template) {
            ...
            function ContactsListVm() {
                ...
                var viewModel = {
                    ...
                    onContactEvent: function (event, payload, param, data) {
                        switch (event) {
                            ...
                            case "on-invite-message":
                                viewModel.onInviteMessage(payload);
                                break;
                            case "on-invite-message-resp":
                                viewModel.onInviteMessageResponse(payload);
                                break;
                    ...
                    onTextMessage: function (sender, data) {
                        try {
                            const invite = JSON.parse(data.text);
                            viewModel.onInviteMessage(sender, invite)
                        } catch (e) {}
                        ...
                        try {
                            if (payload.text.chat === 'rm') {
                                database.del(database.CHATHISTORY, `${appsrvc.pubkeyhash}/${payload.text.pkeyhash}`).then(
                                    ...
                                    err => streembit.notify.error('Error processing deletion of chat history: %j' +err)
                                );
                                return;
                            }
                            ...
                            const data = {
                                ...
                                chat: peermsg.aes256encrypt(appsrvc.datacryptkey, JSON.stringify(payload.text.chat))
                            };
                            database.update(database.CHATHISTORY, data).then(
                                () => streembit.notify.success('Chat history sync received'),
                                err => streembit.notify.error('Error syncing chat: %j' +err)
                            );
                        } catch (err) {
                            streembit.notify.error('Error syncing chat: %j' +err)
                        }
                    },

                    onInviteMessage: function(payload) {
                        try {
                            ...
                            apputils.accept_invite(message, function (result) {
                                ...
                                const webrtcconn = connections.get(contact.name);
                                ...
                                const message = { cmd: defs.PEERMSG_GROUPCHAT_INVITE_RESP, sender: appsrvc.username, text: JSON.stringify(payload) };
                                const msg = peercomm.get_peer_message(contact, message);
                    ...
                    onInviteMessageResponse: function (payload) {
                        try {
                            let data = JSON.parse(payload.text);
                            if (data[peermsg.MSGFIELD.GSID] !== appsrvc.groupchat_id) {
