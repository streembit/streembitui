...
(function () {
    ...
    define(
        ['appsrvc', 'contactlist', 'logger', 'connections', 'uuid', 'peercomm', 'appevents', 'apputils', 'secure-random', 'settings', 'definitions', 'peermsg', 'errhandler', 'errcodes', 'contactsutil', 'uihandler', 'utilities', 'database', './contactsbar.html!text'],
        function (appsrvc, contactlist, logger, connections, uuid, peercomm, appevents, apputils, secrand, settings, defs, peermsg, errhandler, errcodes, contactsutil, uihandler, utilities, database, template) {
            ...
            function ContactsListVm() {
                ...
                function Contact(user_type, params) {
                    ...
                    this.changeToEnvelop = ko.observable(true);
                    if (user_type == "human") {
                ...
                var viewModel = {
                        ...
                        if(list && list.length > 0) {
                            ...
                            if($(window).width() > 767) {
                                if(viewModel.forCont() == 0 && !list.length == 0 && $(window).width() > 425) {
                                    ...
                                    $('.contacts-container').addClass('activate');
                                    $('.contacts-container').css({'width': '240px'});
                            ...
                            for (var i = 0; i < contacts.length; i++) {
                                if (contacts[i].name === account) {
                                    ...
                                    if (isonline) {
                                        ...
                                        contacts[i].changeToEnvelop(true);
                    ...
                    onContactOffline: function(account, isoffline, redirect = false){
                        try{
                            var contacts = viewModel.contacts();
                            for(var i = 0; i < contacts.length; i++){
                                if (contacts[i].name === account) {
                            ...
                            if (redirect) {
                                return appevents.navigate(redirect);
                            }
                        }
                        catch (err) {
                    ...
                    onTextMessage: function (sender, data) {
                        try {
                            if (sender.sender == viewModel.selected_contact_name &&
                                (
                            ...
                            for (var i = 0; i < contacts.length; i++) {
                                if (contacts[i].name == sender.sender) {
                                    ...
                                    contacts[i].changeToEnvelop(false);
                    ...
                    quickChatFromContCall: function (item) {
                        ...
                        try {
                            ...
                            connections.create(item, true, taskid, function (err, webrtcconn) {
                                ...
                                appevents.navigate("contact-chat", options);
                                if($(window).width() < 425) {
                                    $('.calltime_parent').css('right','17px');
                    ...
                    waitscreen: function (msg, cont) {
                        var template = $("#callprogress-from-contactbar-template")[0].innerHTML;
                        var html = $(template);
                        html.find("#wait-msg-tag-quick").text(msg + " " + cont);
                        var waitmsg = html[0].outerHTML;
                        uihandler.blockview(waitmsg);
                    },

                    hangup: function (item) {
                        $('#gradient-titanium-from-contact-bar #hangupCallFromContactPanel').on('click', function () {
                    ...
                    quickCallFromContCall: function (item) {
                        try {
                            ...
                            $('.for-main-content').css({'margin-right': '0'});
                            $('#call').css({'width': '100%'});
                            viewModel.waitscreen("Calling", item.name);
                            viewModel.hangup(item);
