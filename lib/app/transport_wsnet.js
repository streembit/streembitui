...
function TransportWsNet(address, port) {
    ...
    this.protocol = streembit.globals.wsprotocol || "http";
    this.wssocket = 0;
    this.host = 0;
    this.port = port;
    ...
    this.get_account_socket = function() {
        return this.wssocket;
    }

    this.get_socket = function (wshost) {
        return this.wssocket;
    }

    this.get_contact_socket = function (host, port, callback) {
        try {
            var self = this;
            ...
            if (this.host) {
                var server = this.protocol + "http://" + host + ":" + port;
                if (server != this.host) {
                    return callback("The contact WS host and the account WS host must be identical");
                }
            }

            this.create(host, port, function (err) {
                if (err) {
                    return callback(err);
                }
    ...
    this.get_socket_forcontact = function (contact, callback) {
        try {
            if (contact.protocol != defs.TRANSPORT_WS) {
                ...
                var socket = this.get_account_socket();
                if (!socket) {
                ...
                this.get_contact_socket(contact.address, contact.port, function (err, socket) {
                    if (err) {
                        return callback(err);
                    }
                    if (!socket) {
    ...
    this.put = function (key, value, callback) { 
        try {
            ...
            var socket = this.get_account_socket();
            if (!socket) {
                throw new Error("web socket does not exists");
            }
    ...
    this.peer_send = function (contact, data) {
        try {
            ...
            this.get_socket_forcontact(contact, function (err, socket) {
                if (err) {
    ...
    this.find_contact = function (account, public_key, callback) {
        ...
        socket.emit("find_contact", account, function (err, contact) {
