...
function TransportWsNet() {
    ...
    this.socketPingPong = function (socket) {
        this.pingpong = setInterval(() => {
            if (this.isconnected === false) {
                ...
                return this.create(this.host, this.port, (err, payload) => {
                        if (err || !payload.token) {
                            ...
                            // TODO: dispatch event implied to red the connection status btn
            ...
            this.pendingmsgs.set(
                request.txn,
                {
                    ...
                    callback: (err, payload) => {
                        if (err) {
                            throw new Error('failed to send ping with error: ' + (err.message || err));
                        }
                        console.info('Having pong:', payload.pong);
                        if (!payload.pong) {
                            this.isconnected = false;
                            return;
                        }
            ...
            try {
                ...
                clearInterval(this.pingpong);
                throw new Error('ping send error: ' +err.message);
            }
