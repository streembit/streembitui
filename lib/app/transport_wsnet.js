...
function TransportWsNet() {
    ...
    this.create = function (host, port, callback) {
        ...
        try {
            try {
                if (this.openSockets[server]) {
                    this.openSockets[server].onclose = () => {};
                    this.openSockets[server].close();
                }

                socket = new WebSocket(server);
                this.openSockets[server] = socket;
            }
            catch (err) {
    ...
    this.socketPingPong = function () {
        try {
            ...
            if (this.isconnected === false) {
                ...
                if (appsrvc.address && appsrvc.port) {
                    return this.create(this.host, this.port, (err, payload) => {
                        ...
                        this.update_lastseed(true);
                        this.update_connection_status(true);
                    });
                } else {
                    return this.init(err => {
                        if (err) {
                            console.log('Retry. initialisation failed');
                            return;
                        }

                        appsrvc.node = this;
                        appsrvc.address = this.host;
                        appsrvc.port = this.port;
                    })
                }
            }
