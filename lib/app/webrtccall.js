...
(function () {
    ...
    function create_connection() {
        ...
        for(var i=0; i< webrtccall.ice_servers.length; i++){
            // Create a new PeerConnection
            var servers = { iceServers: webrtccall.ice_servers[i] };
    ...
    webrtccall.hangup = function () {
        ...
        try {
            if (mediaStream) {
                ...
                if (document.getElementsByClassName('remote-audio').length) {
                    document.getElementsByClassName('remote-audio').forEach(audioEL => {
