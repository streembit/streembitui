...
(function () {
    ...
    function onRemoteStreamAdded(eventStream) {
        try {
            if (webrtccall.options.calltype == "videocall") {
                ...
                var audio = document.getElementsByTagName('audio')[elnum];
                audio.srcObject = eventStream;

                try {
                    var canvas = document.getElementsByTagName('canvas')[elnum];
                    if (canvas) {
    ...
    webrtccall.hangup = function () {
        ...
        if (webrtccall.connection[webrtccall.options.contact.name]) {
            try {
                webrtccall.connection[webrtccall.options.contact.name].close();
