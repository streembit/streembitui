        ...
        init: function (knockout, jquery) {
            ...
            streembit.notify.error = function (err, param, activitymsg) {
                ...
                if (!activitymsg) {
                    ...
                    var time = Date.now();
                    var isErrorNotDublicated = listOfDangerError.every((err) => {
                        if (err.text !== text) {
                            return true

                        } else {
                           var timeRange =  Math.round(time / 1000) - Math.round(err.time / 1000)
                           if (timeRange >= 2) {
                               return true
                           }
                           return false
                    ...
                    if (isErrorNotDublicated) {
                        listOfDangerError.unshift({
                            text,
                            time
                        })
