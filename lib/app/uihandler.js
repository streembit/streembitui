...
(function () {
    ...
    var uihandler = {
        ...
        isMessageToShow(text) {
            const time = Date.now();
            const isErrorNotDuplicated = listOfDangerError.every(
                err => err.text !== text ? true : (time - err.time) > 600000
            );
    
            return isErrorNotDuplicated
                ? !!listOfDangerError.unshift({ text, time })
                : false;
        },
