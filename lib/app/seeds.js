...
class Seeds {
    ...
    loadConfigSeeds(callback) {
        try {
            if (!settings || !settings.bootseeds || !Array.isArray(settings.bootseeds)) {
                ...
                return callback(errhandler.getmsg(errcodes.UI_INVALID_SEEDS_INSETTINGS_DB));
            }
    ...
    loadAnycastSeeds(callback) {
        ...
        return callback(errhandler.getmsg(errcodes.UI_ANYCAST_SEED_HANDLING_ISNT_IMPLEMENT))
    }
