...
(function () {
    ...
    peercomm.onPeerError = function (payload) {
        try {
            ...
            if (!ishandled) {
                ...
                streembit.notify.error(errhandler.getmsg(errcodes.UI_ONPEERERR_ERR, err));
            }
        }
        catch (e) {
    ...
    function handleAcceptKey(sender, payload, msgtext) {
        try {
            ...
            if (!session) {
                ...
                throw new Error(errhandler.getmsg(errcodes.UI_ACCEPT_KEY_UNABLETO_COMPLETE) + sender);
            }
            ...
            streembit.notify.error(errhandler.getmsg(errcodes.UI_HANDLEACCEPTKEY_ERR, e));
        }
    }
    ...
    function handleKeyExchange(sender, public_key, payload, msgtext) {
        try {
            ...
            if (message.public_key != public_key) {
                ...
                throw new Error(errhandler.getmsg(errcodes.UI_INVALID_PUBLICKEY_FORCONTACT) + sender);
            }
            ...
            if (!plain_data)
                ...
                throw new Error(errhandler.getmsg(errcodes.UI_INVAL_MSG_DATA_FORKEYCONT) + sender);

            var session_symmkey = plain_data.symmetric_key;
            if (!session_symmkey) {
                // throw new Error("invalid session symmetric key for contact " + sender);
                throw new Error(errhandler.getmsg(errcodes.UI_INVAL_SESSION_SYMMKEY_FORCONT) + sender);
            }
            ...
            peersend(contact, encoded_msgbuffer, (err) => {
                if (err) {
                    ...
                    return streembit.notify.error(errhandler.getmsg(errcodes.UI_PEER_SEND_MSGERR, err, true));
                }
            ...
            streembit.notify.error(errhandler.getmsg(errcodes.UI_HANDLE_KEY_EXCHANGE_ERR, e));
        }
    }
    ...
    function handlePing(sender, payload, msgtext) {
        try {
            ...
            if (!contact) {
                ...
                throw new Error(errhandler.getmsg(errcodes.UI_PING_ERR_CONTACT_NOTEXIST));
            }
                ...
                try {
                    ...
                    peersend(contact, encoded_msgbuffer, (err) => {
                        if (err) {
                            ...
                            return streembit.notify.error(errhandler.getmsg(errcodes.UI_HANDLE_PING_PEER_SEND_ERR, err, true));
                        }
                    ...
                    streembit.notify.error(errhandler.getmsg(errcodes.UI_HANDLE_PING_ERR, se, true));
                }
            });
            ...
            streembit.notify.error(errhandler.getmsg(errcodes.UI_HANDLEPING_ERR, e, true));
        }
    }
    ...
    function handlePingReply(sender, payload, msgtext) {
        try {
            ...
            streembit.notify.error(errhandler.getmsg(errcodes.UI_HANDLEPINGREPLY_ERR, e));
        }
    }

    //
    function handleHangupCall(sender, payload, msgtext, msg) {
        try {
            ...
            if (!session) {
                ...
                throw new Error(errhandler.getmsg(errcodes.UI_HANDLEHANGUPCALL_ERR) + sender);
            }
            ...
            streembit.notify.error(errhandler.getmsg(errcodes.UI_HANDLECALLREPLY_ERR, e));
        }
    }
    ...
    function handleShareScreenOffer(sender, payload, msgtext) {
        try {
            ...
            if (!session) {
                ...
                throw new Error(errhandler.getmsg(errcodes.UI_SHARE_SCREEN_OFFER_ERR) + sender);
            }
            ...
            streembit.notify,error(errhandler.getmsg(errcodes.UI_HANDLE_SHARE_SCREEN_OFFER_ERR, e, true));
        }
    }
    ...
    function handleShareScreenReply(sender, payload, msgtext) {
        try {
            ...
            if (!session) {
                ...
                throw new Error(errhandler.getmsg(errcodes.UI_HANDLECALLREPLY_ERR_SESSION_DOESNT_EXIST) + sender);
            }
            ...
            streembit.notify.error(errhandler.getmsg(errcodes.UI_SHARE_SCREEN_REPLY_HANDLER_ERR, e));
        }
    }
    ...
    function handleCall(sender, payload, msgtext) {
        try {
            ...
            if (!session) {
                ...
                throw new Error(errhandler.getmsg(errcodes.UI_SESSION_DOESNT_EXIST) + sender);
            }
            ...
            if (!calltype) {
                ...
                throw new Error(errhandler.getmsg(errcodes.UI_INVALID_CALL_TYPE));
            }
            logger.debug("call type: " + calltype);

            let msg = null, title = null, invite = false;
            if (typeof message.payload !== 'undefined') {
                ...
                if (!message.payload.sid) {
                    ...
                    return streembit.notify.error(errhandler.getmsg(errcodes.UI_INVALID_INVITE_RECEIVES_FROM, message.payload.sender || errhandler.getmsg(errcodes.UI_INTERNATIONAL_MYSTERY_GUY)));
                }
                appevents.dispatch('groupcallevent', 'update-groupcall-obj', message.payload.sid);

                msg = `${message.payload.sender} is inviting you to join a call with ${message.payload.attendees.map(atd => atd.name).join(', ')}`;
                title = 'You Invited to Join a call';
            }

            if (!window.callData) {
                apputils.accept_call(sender, calltype, msg, title,  function (result) {
                    appevents.dispatch("on-appui-event", "hide-contacts-bar");
                ...
                var data = {}
                data[peermsg.MSGFIELD.REQJTI] = payload.jti;
                var encoded_msgbuffer = peermsg.create_typedmsg(peermsg.PEERMSG.CPSY, jti, appsrvc.cryptokey, data, appsrvc.username, sender);
                peersend(contact, encoded_msgbuffer, (err) => {
                    if (err) {
                        return streembit.notify.error(errhandler.getmsg(errcodes.UI_BUSY_NOTIFINCATION, err, true));
                    }
                })
            ...
            streembit.notify.error(errhandler.getmsg(errcodes.UI_HANDLE_CALL_ERR, e));
        }
    }
    ...
    function handleGroupAdd(sender, payload, msgtext) {
        try {
            ...
            if (!session) {
                ...
                throw new Error(errhandler.getmsg(errcodes.UI_HANDLEGROUPADD_ERR) + sender);
            }
            ...
            if (
                ...
                return streembit.notify.error(errhandler.getmsg(errcodes.UI_INVALID_GROUP_ADD_PAYLOAD_RECEIVED));
            }
            ...
            streembit.notify.error(errhandler.getmsg(errcodes.UI_HANDLE_GROUP_ADD_ERR, e))
        }
    }
    ...
    function handleOfferTextChat(sender, payload, data) {
        try {
            ...
            if (!params)
                ...
                throw new Error(errhandler.getmsg(errcodes.UI_INVALID_DATA_FROM) + sender);
            ...
            streembit.notify.error(errhandler.getmsg(errcodes.UI_HANDLE_OFFER_TEXT_CHAT_ERR, e, true));
        }
    }

    function handelCallBusyNotification(sender, payload, data) {
        console.log('Show notification Call Busy')
        var params = JSON.parse(data);
        streembit.notify.info("Contact " + sender + " is Busy, Please try later");
        closeWaithandler(params['request_jti'], false)
    ...
    function handleFileInit(sender, payload, data) {
        try {
            ...
            if (!session) {
                ...
                throw new Error(errhandler.getmsg(errcodes.UI_HANDLEFILEINIT_ERR) + sender);
            }

            var params = JSON.parse(data);
            if (!params || !params.file_name || !params.file_size)
                // throw new Error("handleFileInit error, invalid file data from " + sender);
                throw new Error(errhandler.getmsg(errcodes.UI_INVALID_FILE_DATA_FROM) + sender);
            ...
            streembit.notify.error(errhandler.getmsg(errcodes.UI_HANDLE_FILE_INIT_ERROR, e));
        }
    }
    ...
    function handleAddContactRequest(sender, payload, msgtext) {
        try {
            ...
            streembit.notify.error(errhandler.getmsg(errcodes.UI_HANDLEADDCONTACTREQUEST_ERR, e));
        }
    }
    ...
    function handleAddContactAcceptReply(sender, payload, msgtext) {
        try {
            ...
            if (sender != account) {
                ...
                throw new Error(errhandler.getmsg(errcodes.UI_INVAL_ACCOUNT_SENDER_DONT_MATCH));
            }
    ...
    function handleAddContactDenyReply(sender, payload, msgtext) {
        try {
            ...
            streembit.notify.error(errhandler.getmsg(errcodes.UI_HANDLEADDCONTACTDENYREPLY_ERR, e));
        }
    }
    ...
    function handleSymmMessage(sender, payload, msgtext) {
        try {
            ...
            switch (data.cmd) {
                case defs.PEERMSG_TXTMSG:
                    try {
                        ...
                        streembit.notify.error(errhandler.getmsg(errcodes.UI_ERR_IN_HANDLING_CHAT_MSG, e));
                    }
                    break;
                ...
                case defs.PEERMSG_CALL_WEBRTC:
                    if(!window.isCallReady) {
                        // wait until the stream is ready, because there will be a little delay for the first time audio view is loaded
                        // this will add the signals in Queue so they will not be wasted, and dispatch them when the listener is ready
                        var signalToBeFired = function(fn, context, params) {
                            return function() {
                                fn.apply(context, params);
                            };
                        }
                        var signal = signalToBeFired(appevents.dispatch, this, ["on-call-webrtc-signal", data]);
                        window.signalsQueue.push(signal);
                    } else {
                ...
                case defs.PEERMSG_DATA_WEBRTC:
                    appevents.dispatch("on-webrtc-data-signal", data, sender);
                    break;
                ...
                case defs.ERROR_TEXT_CHAT:
                    appevents.dispatch('on-text-chat-error');
                    break;
            ...
            streembit.notify.error(errhandler.getmsg(errcodes.UI_HANDLE_SYMM_MESSAGE_ERR, e));
        }
    }
    ...
    peercomm.onPeerMessage = function (message) {
        try {
            ...
            if (!message.payload) {
                ...
                return streembit.notify.error(errhandler.getmsg(errcodes.WS_PEER_MSG_ERR, null, true));
            }
    ...
    peercomm.processPeerMsg = function (data, info) {
        try {
            ...
            if (!public_key) {
                ...
                throw new Error(errhandler.getmsg(errcodes.UI_NO_PUBLIC_KEY_FOR_CONTACT) + sender);
            }
            ...
            switch (message.sub) {
                ...
                case peermsg.PEERMSG.OFTX:
                    ...
                    break;
                case peermsg.PEERMSG.CPSY:
                    handelCallBusyNotification(sender, payload, message.data)
            ...
            streembit.notify.error(errhandler.getmsg(errcodes.UI_ERR_IN_HANDLING_PEER_MSG, err, true));
        }
    }
        ...
        return new Promise(function (resolve, reject) {
            try {
                ...
                try {
                    ...
                    streembit.notify.error(errhandler.getmsg(errcodes.UI_PEER_SEND_MSGERR, pserr, true));
                }
            }
            catch (err) {
                reject(err);
    ...
    peercomm.send_peer_message = function (contact, message) {
        ...
        try {
            ...
            if (!contact) {
                ...
                throw new Error(errhandler.getmsg(errcodes.UI_INVALID_CONTACT_PARAMS));
            }
            if (!message) {
                // throw new Error("invalid message parameter");
                throw new Error(errhandler.getmsg(errcodes.UI_INVALID_MESSAGE_PARAMS));
            }

            if (!appsrvc.cryptokey) {
                // throw new Error("invalid use crypto key");
                throw new Error(errhandler.getmsg(errcodes.UI_INVALID_USE_CRYPTO_KEY));
            }
            ...
            if (!session || !session.accepted || !session.symmetric_key) {
                ...
                throw new Error(errhandler.getmsg(errcodes.UI_CONTACT_SESSION_KEY_DAESNT_EXISTS));
            }
            var jti = create_id();
            ...
            peersend(contact, encoded_msgbuffer, (err) => {
                if(err) {
                    ...
                    streembit.notify.error(errhandler.getmsg(errcodes.UI_SEND_PEER_MSG_ERR, err, true));
                }
            });
        }
        catch (e) {
            timer = setTimeout(function(){
                if(session == undefined && window.count == true){
                    window.count = false;
                    streembit.notify.error(errhandler.getmsg(errcodes.UI_SEND_PEER_MSG_ERR, e, true));
                }
            }, 10000);
    ...
    peercomm.get_peer_message = function (contact, message) {
        try {
            ...
            streembit.notify.error(errhandler.getmsg(errcodes.UI_GET_PEER_MSG_ERR, e, true));
        }
    }
    ...
    peercomm.hangup_call = function (contact, group, msg) {
    ...
    peercomm.publish_user = function (symcryptkey, key, public_key, transport, address, port, type, callback) {
        try {
            if (!callback || typeof callback != "function") {
                ...
                return callback(errhandler.getmsg(errcodes.UI_PUBLISH_USER_ERR))
            }

            //  publishing user data
            if (!public_key || !address || (transport == "tcp" && !port)) {
                // return callback("invalid parameters at publish user");
                return callback(errhandler.getmsg(errcodes.UI_INVALID_PARAMS_PUBLISH_USER));
            }
            ...
            if (transport == defs.TRANSPORT_WS) {
                ...
                if (!symcryptkey) {
                    ...
                    return callback(errhandler.getmsg(errcodes.UI_INVALID_CONN_KEY_PUBLISH_USER));
                }
            ...
            callback(errhandler.getmsg(errcodes.UI_PUBLISH_PEER_USER_ERR) + e.message);
        }
    }
    ...
    peercomm.send_contact_offer = function (account, user_pkhash, user_public_key, contact_bs58public_key, connsymmkey, transport, address, port, type, callback) {
        try {
            if (!callback || typeof callback != "function") {
                ...
                return callback(errhandler.getmsg(errcodes.UI_OFFER_CONTACT_ERR))
            }
            ...
            if (!account || !user_pkhash || !user_public_key || !address || !contact_bs58public_key || !connsymmkey || (transport == "tcp" && !port)) {
                ...
                return callback(errhandler.getmsg(errcodes.UI_OFFER_ERR_INVAL_PARAMS));
            }
            ...
            var key_raw = user_pkhash + "/" + contact_pkhash;
            var key_buffer = new Buffer(key_raw);
            var key = createHash('rmd160').update(key_buffer).digest('hex');

            var payload = {};
            payload.type = peermsg.MSGTYPE.CAMSG;
            payload[peermsg.MSGFIELD.CIPHER] = cipher;
            payload[peermsg.MSGFIELD.KEYDATA] = key_raw;
