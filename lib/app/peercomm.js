...
(function () {
    ...
    function handleGroupcallAdd(sender, payload, msgtext) {
        try {
            const msg = JSON.parse(msgtext);
            if (msg[peermsg.MSGFIELD.CALLT] === defs.CALLTYPE_AUDIO) {
                appevents.dispatch("oncontactevent", "on-groupcall-add", msg['payload']);
            }
        } catch (err) {
            throw new Error(err);
        }
    ...
    function handleGroupcallReady(sender, payload, msgtext) {
        try {
            const msg = JSON.parse(msgtext);
            if (msg[peermsg.MSGFIELD.CALLT] === defs.CALLTYPE_AUDIO) {
                appevents.dispatch("oncontactevent", "on-groupcall-ready", msg);
            }
        } catch (err) {
            throw new Error(err);
        }
        ...
        try {
            ...
            switch (message.sub) {
                ...
                case peermsg.PEERMSG.GCAD:
                    handleGroupcallAdd(sender, payload, message.data);
                    break;
                case peermsg.PEERMSG.GCRY:
                    handleGroupcallReady(sender, payload, message.data);
                    break;
    ...
    peercomm.group_callready = function(contact, type, sid, pkhash) {
        try {
            ...
            var jti = create_id();
            var encoded_msgbuffer = peermsg.create_typedmsg(peermsg.PEERMSG.GCRY, jti, appsrvc.cryptokey, data, appsrvc.username, account);
