...
(function () {
    ...
    function handleGroupChatPing(sender, payload, msgtext) {
        try {
            var message = JSON.parse(msgtext);
            var public_key = message[peermsg.MSGFIELD.ECDHPK];
            ...
            // send Group Chat Ping reply message
            var data = {};
            data[peermsg.MSGFIELD.REQJTI] = payload.jti;
            data[peermsg.MSGFIELD.ECDHPK] = appsrvc.publickeyhex;

            var jti = create_id();
            var encoded_msgbuffer = peermsg.create_typedmsg(peermsg.PEERMSG.GPRE, jti, appsrvc.cryptokey, data, appsrvc.username, sender);

            peersend(attendee, encoded_msgbuffer, (err) => {
                if (err) {
                    return streembit.notify.error("Handle group chat ping error %j", err, true);
                }
            });
        }
        catch (err) {
    ...
    function handleGroupChatPingReply(sender, payload, msgtext) {
        try {
            var data = JSON.parse(msgtext);
            //  must have the request_jti field
            var jti = data[peermsg.MSGFIELD.REQJTI];
            ...
            // !! sender string can find unexpected match with existing contact name !!
            if (!list_of_sessionkeys[sender]) {
                list_of_sessionkeys[sender] = {};
            }
            list_of_sessionkeys[sender].contact_public_key = public_key;
            logger.debug("handleGroupChatPingReply public_key: " + public_key + " for " + sender);
        ...
        try {
            ...
            switch (data.cmd) {
                case defs.PEERMSG_TXTMSG:
                    try {
                        const add_attendee = JSON.parse(data.text);
                        const attendee = add_attendee.attendee;
                        // initiate connection here
                        // and send event back to component
                        if (!add_attendee.sid) {
                            return streembit.notify.error("Invalid groupchat ID");
                        }

                        peercomm.ping(attendee, false, 10000, PEERMSG.GPNG)
                            .then(() => {
                                return peercomm.get_contact_session(attendee);
                            })
                            .then(() => {
                                appsrvc.groupchat_id = add_attendee.sid;
                                // TODO: send event to initiate audio call
                ...
                case defs.PEERMSG_GROUPCHAT_INVITE:
                    try {
                        ...
                        streembit.notify.error("Error in handling invite message %j", e);
                    }
                    break;
                case defs.PEERMSG_GROUPCHAT_INVITE_RESP:
                    try {
                        appevents.dispatch("oncontactevent", "on-invite-message-response", data);
                    }
                    catch (e) {
                        streembit.notify.error("Error in handling invite message %j", e);
                    }
        ...
        try {
            ...
            switch (message.sub) {
                ...
                case peermsg.PEERMSG.PREP:
                    ...
                    break;
                case peermsg.PEERMSG.GPNG:
                    handleGroupChatPing(sender, payload, message.data);
                    break;
                case peermsg.PEERMSG.GPRE:
                    handleGroupChatPingReply(sender, payload, message.data);
                    break;
    ...
    peercomm.ping = function (contact, showprogress, timeout, msgtype = null) {
        return new Promise(function (resolve, reject) {
            try {
                ...
                var jti = create_id();
                var encoded_msgbuffer = peermsg.create_typedmsg(msgtype || peermsg.PEERMSG.PING, jti, appsrvc.cryptokey, data, appsrvc.username, account);
    ...
    peercomm.hangup_call = function (contact) {
    ...
    peercomm.call = function (contact, type, showprogress) {
