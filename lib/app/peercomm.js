...
(function () {
    ...
    function handleHangupCall(sender, payload, msgtext) {
        try {
            ...
            events.emit(events.TYPES.ONAPPNAVIGATE, defs.CMD_HANGUP_CALL, sender);
        ...
        try {
            ...
            streembit.UI.accept_sharescreen(sender, function (result) {
                ...
                if (result) {
                    ...
                    events.emit(events.TYPES.ONAPPNAVIGATE, defs.CMD_RECIPIENT_SHARESCREEN, null, uioptions);
                }
            });
        }
        catch (e) {
    ...
    function handleFileInit(sender, payload, data) {
        try {
            ...
            streembit.UI.accept_file(sender, obj.file_name, obj.file_size, function (result) {
                ...
                if (result) {
                    events.emit(events.TYPES.ONAPPNAVIGATE, defs.CMD_FILE_INIT, sender, obj);
                }
            });
    ...
    percomm.get_account_messages = function (msgkey, page, start) {
        try {
            ...
            streembit.Node.get_range(querystring, function (err, result) {
                if (err) {
                    return events.emit(defs.EVENT_ERROR, streembit.Error.Codes.FINDMESSAGES, err);
                }
