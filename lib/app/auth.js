...
(function () {
    ...
    function validateMessage(params, contact, callback) {
        ...
        try {
            var payload = slib.message.getpayload(params.value);
            if (!payload || !payload.data || !payload.data.type) {
                return callback("validateMessage error invalid payload");
            }

            if (payload.data.type == slib.message.MSGTYPE.PUBPK ||
                payload.data.type == slib.message.MSGTYPE.UPDPK ||
                payload.data.type == slib.message.MSGTYPE.DELPK) {
                if (!payload.data[slib.message.MSGFIELD.PUBKEY]) {
            ...
            if (payload.data.type == slib.message.MSGTYPE.DELMSG) {
        ...
        node.get(account_key, function (err, value) {
            try {
                if (err) {
                    ...
                    var stored_payload = slib.message.getpayload(value);
                    var stored_pkkey = stored_payload.data[slib.message.MSGFIELD.PUBKEY];
                    if (!stored_pkkey) {
                    ...
                    if (payload.data.type == slib.message.MSGTYPE.PUBPK) {
                        if (payload.data[slib.message.MSGFIELD.PUBKEY] != stored_pkkey) {
                    ...
                    if (payload.data.type == slib.message.MSGTYPE.PUBPK ||
                        payload.data.type == slib.message.MSGTYPE.UPDPK ||
                        payload.data.type == slib.message.MSGTYPE.DELPK ||
                        payload.data.type == slib.message.MSGTYPE.OMSG ||
                        payload.data.type == slib.message.MSGTYPE.DELMSG) {

                        var decoded_msg = slib.message.decode(params.value, stored_pkkey);
                        if (!decoded_msg) {
    ...
    auth.expireHandler = function(data, callback) {
        try {
            ...
            // get the payload
            var payload = slib.message.getpayload(msgobj.value);
            ...
            if (data.key.indexOf("/") == -1) {
                ...
                if ((!payload || !payload.data || !payload.data.type) || payload.data.type == slib.message.MSGTYPE.DELPK) {
                ...
                if (data && data.key && (typeof data.key === 'string') && data.value && (typeof data.value === 'string')) {
                    try {
                        if (data.key.indexOf(key) > -1) {
                            ...
                            if (jsonobj.value) {
                                var payload = slib.message.getpayload(jsonobj.value);
                                if (payload.data.type != slib.message.MSGTYPE.DELMSG) {
