...
(function () {
    ...
    function WebRTCData() {
        ...
        function create_connection(contact) {
            ...
            logger.debug('WebRTC: connection for ' + contact.name );
        ...
        webrtcdata.onSignalReceive = function (data, sender) {
            ...
            if (sender === webrtcdata.contact.name) {
                logger.debug('WebRTC: received signal type: %s', data.type);

                // Route signal based on type
                if (data.sdp) {
                    onSdpSignalReceived(webrtcdata.connection, data.sdp);
                }
                else if (data.candidate) {
                    onCandidateSignalReceived(webrtcdata.connection, data.candidate);
                }
                else {
        ...
        function closeDataChannel() {
            ...
            try {
                if (webrtcdata.connections.length) {
                    for(index in webrtcdata.connections) {
                        webrtcdata.connections[index].close();
                        webrtcdata.connections[index] = null;
                    }
        ...
        // function get_webrtc_connection(contact) {
        //     var exists = false;
        //     var contactIndex = 0;
        //     for(var index in webrtcdata.connections) {
        //         if(webrtcdata.connections[index].name == contact.name) {
        //             exists = true;
        //             contactIndex = index;
        //         }
        //     }
        ...
        //     if (!exists) {
        //         webrtcdata.connections.push(create_connection(contact))
        //         return webrtcdata.connections[webrtcdata.connections.length - 1];
        //     } else {
        //         return webrtcdata.connections[contactIndex];
        //     }
        // }
        ...
        webrtcdata.createchannel = function (contact, issender) {
            try {
                webrtcdata.is_outgoing_call = issender;

                webrtcdata.contact  = webrtcdata.contacts.filter(function(cont) {
                    return cont.name == contact.name;
                })[0];

                webrtcdata.connection = webrtcdata.connections.filter(function(conn) {
                    return conn.name == webrtcdata.contact.name;
                })[0];
                ...
                //get_webrtc_connection(webrtcdata.contact);
            }
            catch (err) {
        ...
        webrtcdata.invite = function (contact, issender) {
            return new Promise((resolve, reject) => {
                webrtcdata.onrcvstatechange = function () {
                    ...
                    if (readyState === 'open') {
                        ...
                        webrtcdata.connected = false;
                        try {
                            webrtcdata.connection.onclosed(webrtcdata.contact.name);
                ...
                // add the contact
                addContact(contact);
                //careate and add the contact
                addConnection(contact);
                // create the channel
                webrtcdata.createchannel(contact, issender);
            })
        };

        var addContact = function(contact) {
            var exists = false;
            for(var index in webrtcdata.contacts) {
                if(webrtcdata.contacts[index].name == contact.name) {
                    exists = true;
                }
            }

            if (!exists) {
                webrtcdata.contacts.push(contact)
            }
        }

        var addConnection = function(contact) {
            var exists = false;
            for(var index in webrtcdata.connections) {
                if(webrtcdata.connections[index].name == contact.name) {
                    exists = true;
            ...
            if (!exists) {
                var conn = create_connection(contact)
                conn.name = contact.name;
                webrtcdata.connections.push(conn);
            }
        }
