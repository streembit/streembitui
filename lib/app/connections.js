...
(function () {
    ...
    handler.create_receiver = function (contact) {
        ...
        webrtcdata.createchannel(false, function () {
            ...
            appevents.dispatch('groupchatevent', 'add-webrtcconn', contact, webrtcdata);
        });
    };
    ...
    handler.create = function (contact, issender, taskid, callback) {
        ...
        const createChannel = function () {
            const webrtcdata = new WebRTCData(contact);
            if (webrtcdata.connection) {
                webrtcdata.connection.close();
            }
            webrtcdata.createchannel(issender, function (err) {
                if (err) {
                    console.log('CREATECH ERR::', err);
                    createOpt.err = err;
                }
                else {
                    list_of_webrtc_connections[contact.name] = webrtcdata;
                    webrtcdata.onclosed = handler.onclosed;
                    clearInterval(createOpt.int);
                    callback(null, webrtcdata);
                }
            });
        };

        createOpt.int = setInterval(() => {
            if (--createOpt.maxTry >= 0) {
                logger.debug('Recreating communication channel..');
                createChannel();
            } else {
                createOpt.maxTry = 2;
                clearInterval(createOpt.int);
                console.log('FINALERR', createOpt.err);
                throw new Error(createOpt.err || 'Create channel timed out');
            }
        }, 5000);
    };
