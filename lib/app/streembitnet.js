...
(function () {
    ...
    function netfactory(transport) {
        if ((streembit && streembit.globals && streembit.globals.nwmode) && transport == defs.TRANSPORT_TCP) {
            ...
            return Object.assign(new WsNet(), settings.lastseed);
        }
    }
            ...
            peernet.connect(settings.transport, function (err) {
                if (!err) {
                    if (settings.transport == defs.TRANSPORT_WS) {
                        ...
                        peernet.netclient.update_lastseed(err);
                ...
                // it was an error
                if (settings.transport == defs.TRANSPORT_WS && typeof settings.lastseed.host !== 'undefined') {
                    peernet.netclient.update_lastseed(err);
                    connect_via_ws();
                }
                else if (settings.transport == defs.TRANSPORT_WS || (settings.transport == defs.TRANSPORT_TCP && (!settings.wsfallback && settings.ask_about_wsfallback))) {
