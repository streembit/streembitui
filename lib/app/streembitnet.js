...
(function () {
    ...
    peernet.getwshandler = function (address, port) {
        ...
        if (!peernet.wshandler[key]) {
            peernet.wshandler[key] = new WsNet();
            peernet.wshandler[key].host = address;
            peernet.wshandler[key].port = port;
        }
    ...
    peernet.init = function () {
        ...
        return new Promise((resolve, reject) => {
            ...
            peernet.connect(settings.transport, function (err) {
                if (!peernet.netclient.monitoring) {
                    peernet.netclient.monitor();
                }
                ...
                if (!err) {
                    if (settings.transport == defs.TRANSPORT_WS) {
                        ...
                        peernet.netclient.update_connection_status(!err);
