...
(function () {
    ...
    handler.get_contact_by_pkhash = function (pkhash) {
        var contact = null;
        var contacts = list_of_contacts[appsrvc.username];
        if (contacts) {
            for (var i = 0; i < contacts.length; i++) {
                if (contacts[i].pubkeyhash === pkhash) {
                    contact = contacts[i];
                    break;
                }
            }
    ...
    handler.updateDB = function (contact) {
        ...
        database.ContactsDB.update_contact(appsrvc.username, contact).then(
            ...
            function (err) {
                ...
                streembit.notify.error(errhandler.getmsg(errcodes.UI_DB_UPDATE_CONTACT_ERR, err, true));
            }
        );
    }
    ...
    handler.update_contact = function (contact, callback) {
        ...
        database.ContactsDB.update_contact(appsrvc.username, contact).then(
            ...
            function (err) {
                ...
                streembit.notify.error(errhandler.getmsg(errcodes.UI_DB_UPDATE_ADD_CONTACT_ERR, err, true));
            }
        );
    ...
    handler.remove = function (name, callback) {
        ...
        database.ContactsDB.delete_contact(appsrvc.username, name, function (err) {
            if (err) {
                ...
                return streembit.notify.error(errhandler.getmsg(errcodes.UI_DELETE_CONTACT_ERR, err));
            }            
