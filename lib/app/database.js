...
(function () {
    ...
    database.CHATHISTORY = 'chathistory';
            ...
            if (!store) {
                switch (store_name) {
                    ...
                    default:
                        ...
                        throw new Error(errhandler.getmsg(errcodes.UI_FAILED_CREATE_OBJECT_STORE) + database.BLOCKCHAINDB);
                }
            }
        }
        catch (cerr) {
            // throw new Error("error in creating object store " + database.BLOCKCHAINDB + ": " + cerr.message);
            throw new Error(errhandler.getmsg(errcodes.UI_ERR_CREATING_OBJECT_STORE) + database.BLOCKCHAINDB + ": " + cerr.message);
        }
    ...
    function create_objectstores() {
        ...
        try {
            db.createObjectStore(database.CHATHISTORY, { keyPath: 'key' });
        }
        catch (err) {
            console.log("createObjectStore " + database.CHATHISTORY + " error: " + err.message);
        }
    }
            ...
            function (data) {
                try {
                    if (data && data.key && data.key == key) {
                        ...
                        cb(errhandler.getmsg(errcodes.UI_COULDNT_FIND_DATA_FORKEY));
                    }
                }
                catch (err) {
                    return cb(err);
    ...
    MainDB.put = function (key, datastr, cb) {
        ...
        if (!data || !data.key || key != data.key) {
            ...
            throw new Error(errhandler.getmsg(errcodes.UI_INVALID_DATA_KEY_MUST_EXISTS_INDATA_OBJECT))
        }
    ...
    database.ContactsDB.get_contacts = function (account, cb) {
        if (!account) {
            ...
            return cb(errhandler.getmsg(errcodes.UI_INVALID_ACCOUNT_PARAMS));
        }
    ...
    database.ContactsDB.delete_contact = function (account, name, cb) {
        ...
        if (!name) {
            ...
            return cb(errhandler.getmsg(errcodes.UI_INVALID_NAME_PARAMS));
        }
