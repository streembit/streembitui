...
(function () {
    ...
    Object.defineProperty(usrobj, "mnemonic", {
        get: function () {
            if (!m_mnemonic) {
                m_mnemonic = usrobj.create_mnemonic(usrobj.password);
            }
            return m_mnemonic;
        },
    ...
    usrobj.create_mnemonic = function (password) {
        if (!password) {
            throw new Error("create_mnemonic error, invalid parameter");
        }
        var buffer = new Buffer(password);
        var phentropy = createHash("sha256").update(buffer).digest("hex");
        var mnemonic = bip39.entropyToMnemonic(phentropy);
        return mnemonic;
    }    
        ...
        try {
            ...
            getCryptPassword(pwdhash, salt, function (err, pkbdf2hash) {
                try {
                    ...
                    usrobj.salt = salt;
                    usrobj.pkbdf2_password_hash = pkbdf2hash;
                    usrobj.mnemonic = usrobj.create_mnemonic(password);
