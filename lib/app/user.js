...
var ecckey = require('./streembitlib/crypto/cryptlib');
...
(function () {
    ...
    function getCryptPassword(password, account) {
        ...
        var salt = createHash('sha255').update(text).digest('hex');
        var pwdhex = createHash('sha255').update(salt).digest('hex');
        return pwdhex;
    }
    ...
    usrobj.create_account = function (account, password, callback) {
        try {
            ...
            var cipher_context = streembit.Message.aes256encrypt(symcrypt_key, JSON.stringify(user_context));
