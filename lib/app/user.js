    ...
    usrobj.create_key = function (password, mnemonic, callback) {
        ...
        try {
            ...
            if (mnemonic && !usrobj.mnemonic) {
                throw new Error("The mnemonic restore requires the value of mnemoic to be set priot calling the create_key procedure.");
            }
            ...
            getCryptPassword(pwdhash, salt, function (err, pkbdf2hash) {
                try {
                    ...
                    if (key.publicKeyHex != user.public_key) {
                        streembit.notify.error("Error in initializing the account, most likely an incorrect password or the seed entropy is incorrect");
                        return false;
                    }
                    ...
                    usrobj.salt = salt;
                    usrobj.pkbdf2_password_hash = pkbdf2hash;
                    usrobj.mnemonic = usrobj.create_mnemonic(usrobj.seedentropy);
