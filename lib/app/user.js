...
(function () {
    ...
    function getCryptPassword(password, user, actionType, cb1, cb2) {
        cb2 = cb2 || null;
        var saltbuffer = new Buffer(user.salt);
        var pwdbuffer = new Buffer(password.hash);
        ...
        window.crypto.subtle.importKey(
            ...
            cb1(null, { pass: password.pass, hash: pkbdf2hash }, user, actionType, cb2);
        })
        .catch(function (err) {
            cb2 ? cb2(err) : cb1(err);
        });
    ...
    function cbCryptPassword(err, password, user, actionType, cb) {
        try {
            if (err) {
                cb ? cb(err) : streembit.notify.error(err);
                return false;
            }
            ...
            var plain_text;
            try {
                plain_text = peermsg.aes256decrypt(password.hash, user.cipher);
            }
            catch (err) {
                if (err.message && err.message.indexOf("decrypt") > -1) {
                    return cbCryptPassword("Account initialize error, most likely an incorrect password was entered");
                }
                else {
                    return cbCryptPassword("Account initialize error: %j", err);
                }
                return false;
            }
            ...
            try {
                ...
                if (!accountpayload || !accountpayload.seedentropy || !accountpayload.privatekey ||
                    ...
                    return cbCryptPassword("invalid password or invalid user object stored");
                }
            }
            catch (e) {
            ...
            usrobj.password = password.pass;
            usrobj.salt = user.salt;
            usrobj.pkbdf2_password_hash = password.hash;
            ...
            cbCryptPassword("Account initialize error: " + err.message);
        }
    }
    ...
    usrobj.initialize_account = function (user, password, callback) {
        try {
            ...
            getCryptPassword({ pass: password, hash: pwdhash }, user, 'initialize', cbCryptPassword, callback);
        }
        catch (err) {
            callback("Account initialize error. Select a saved account and enter the valid password.");
        }
    };
    ...
    usrobj.restore = function (password, user, callback) {
        try {
            ...
            getCryptPassword({ pass: password, hash: pwdhash }, user, 'restore', cbCryptPassword, callback);
        }
        catch (e) {
