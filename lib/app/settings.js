...
const addconfig = require('../config.app.json!');
    ...
    var settings = {
        ...
        getwspeers: function () {
            try {
                ...
                function errorhandler(errmsg, url) {
                    streembit.notify.error(errmsg + " URL: " + url, null, true);
                }

                function compareBootseeds(s1, s2) {
                    return s1.length === s2.length && s1.every(
                        (v,i) =>
                            v.host === s2[i].host && (v.port || null) == (s2[i].port || null)
                    );
                }

                if (!settings_data.bootseeds.length || compareBootseeds(settings_data.bootseeds, config.appconfig.bootseeds)) {
                    logger.info("Populating WS peer list from Github repository");
                    var url = settings_data.kadence_peerlist;
                    $.ajax({
                        url: url,
                        type: 'GET',
                        success: function (res) {
                            try {
                                var seedlist = (res && typeof res !== "object") ? JSON.parse(res) : res;
                                ...
                                errorhandler(errhandler.getmsg(errcodes.WS_POPULATING_PEER_LIST) + err.message, url);
                            }
                        },
                        error: function () {
                            // errhandler("Error in populating WS peer list from Github repository", url);
                            errorhandler(errhandler.getmsg(errcodes.WS_POPULATING_PEER_LIST_FROM_GITREPO, url));
                        }
                    });
                }
            }
            catch (err) {
                errorhandler("Error in populating WS peer list from Github repository " + err.message);
            }
        },
        ...
        getdata: function (callback) {
            database.get(database.SETTINGSDB, "settings").then(
                function (result) {
                    ...
                    if (!val.kadence_peerlist) {
                        val.kadence_peerlist = config.appconfig.kadence_peerlist;
                    }
