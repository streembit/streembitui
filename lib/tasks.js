...
class Task {
    ...
    execute(time) {
        try {
            ...
            streembit.notify.error(errhandler.getmsg(errcodes.UI_TASK_EXECUTE_ERR, err, true));
        }
    }
}
...
class TaskHandler {
    ...
    addTask(name, interval, callback) {
        if (!name || !interval || !callback || interval < 1000 || typeof callback != "function") {
            ...
            throw new Error(errhandler.getmsg(errcodes.UI_INVALID_TASK_PARAMS));
        }
    ...
    taskHandler() {
        try {
            ...
            streembit.notify.error(errhandler.getmsg(errcodes.UI_TASKHANDLER_ERR, err, true));
        }
    }
